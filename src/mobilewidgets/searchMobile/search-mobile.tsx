...
import { useSearchIsClicked } from "../../header/mobile/header-mobile";
...
export const useSearchStore = create<TSearchStore>((set) => ({
  searchRequest: "",
  setWord: (searchRequest: string) => set(() => ({ searchRequest: searchRequest  })),
}))
    ...
    <div className="searchM">
      <div className='search__wrapperM'>
        ...
        <div className='search__resultsM'>
            <div>
                {searchProductsResult !== null ? searchProductsResult.map((product: Product) => <SearchProductMobile setIsClicked={setIsClicked} id={product.id} name={product.name}/>) : popularArrayForSearch.map((product: Product) => <SearchProductMobile setIsClicked={setIsClicked} name={product.name} />)}
        ...
        <Link to="/showallresults">
         <div onClick={()=>{setWord(debouncedValue||""); setSearchIsClosed() }}>показать все результаты</div>
        </Link>
      </div>
    </div>,
    document.body
